...
package org.roger600.lienzo.client.toolbox;
...
public class ToolboxButton {
    ...
    private static final double SHAPE_ALPHA_UNFOCUSED = 0.5;
    private static final double SHAPE_ALPHA_FOCUSED = 1;
    private static final double DECORATOR_STROKE_UNFOCUSED = 0;
    private static final double DECORATOR_STROKE_FOCUSED = 1;
    ...
    public ToolboxButton setX(final double x) {
        getShape().setX(x);
        return this;
    }

    public ToolboxButton setY(final double y) {
        getShape().setY(y);
        return this;
    }

    public ToolboxButton show() {
        if (!isVisible()) {
            setShapeAlpha(SHAPE_ALPHA_UNFOCUSED);
        }
        return this;
    }

    public ToolboxButton hide() {
        if (isVisible()) {
            setShapeAlpha(0);
            hideDecorator();
        }
    ...
    private WiresShape build(final IPrimitive<?> shape,
        ...
        decorator = TestsUtils.rect(new MultiPath(),
                                    size + pad,
                                    size + pad,
        ...
        handlerRegistrationManager.register(
                shape.addNodeMouseEnterHandler(new NodeMouseEnterHandler() {
                                                   ...
                                                   public void onNodeMouseEnter(NodeMouseEnterEvent event) {
                                                       ToolboxButton.this.onButtonMouseEnter(shape,
        ...
        handlerRegistrationManager.register(
                shape.addNodeMouseExitHandler(new NodeMouseExitHandler() {
                                                  ...
                                                  public void onNodeMouseExit(NodeMouseExitEvent event) {
                                                      ToolboxButton.this.onButtonMouseExit(shape,
                                                                                           getLocation(wiresShape),
                                                                                           wiresShape.getGroup().getAbsoluteLocation(),
                                                                                           event.getHumanInputEvent().getClientX(),
                                                                                           event.getHumanInputEvent().getClientY());
                    ...
                    shape.addNodeMouseClickHandler(new NodeMouseClickHandler() {
                                                       ...
                                                       public void onNodeMouseClick(NodeMouseClickEvent event) {
                                                           ToolboxButton.this.onButtonMouseClick(shape,
                                                                                                 ToolboxButton.this.getLocation(wiresShape),
                                                                                                 wiresShape.getGroup().getAbsoluteLocation(),
                                                                                                 event.getHumanInputEvent().getClientX(),
                                                                                                 event.getHumanInputEvent().getClientY());
                                                       }
    ...
    private boolean isVisible() {
        return getShape().getGroup().getAlpha() > 0;
    }
    ...
    private void focusPrimitive() {
        primitive.getGroup().setAlpha(SHAPE_ALPHA_FOCUSED);
    }

    private void unfocusPrimitive() {
        primitive.getGroup().setAlpha(SHAPE_ALPHA_UNFOCUSED);
    }

    private void showDecorator() {
        decorator.setStrokeAlpha(DECORATOR_STROKE_FOCUSED);
    }

    private void hideDecorator() {
        decorator.setStrokeAlpha(DECORATOR_STROKE_UNFOCUSED);
    }
